...
export function resolveEoqGroup(description?: string | null, category?: string | null): string {
  ...
  if (cat) {
    ...
    if ((EOQ_GROUP_ORDER as readonly string[]).includes(cat)) return cat
  }
...
export function groupEoqItems<T extends { description?: string | null; category?: string | null }>(
  items: T[],
): EoqGroupSection<T & { display_number: number }>[] {
  const buckets = new Map<string, T[]>()
  for (const item of items) {
    const group = resolveEoqGroup(item.description, item.category)
